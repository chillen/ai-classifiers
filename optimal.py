...
def getClassValue(Ea, Eb, Ma, Mb, x):
    mbChunk = np.multiply(np.transpose((np.subtract(x, Mb))), np.multiply(np.linalg.pinv(Eb), np.subtract(x,Mb)))
    maChunk = np.multiply(np.transpose((np.subtract(x, Ma))), np.multiply(np.linalg.pinv(Ea), np.subtract(x,Ma)))
    return log(Eb) - log(Ea) + mbChunk - maChunk

def getClassifier(trainingSet):
